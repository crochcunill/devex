							...
							ProposalsService.assign(ppp.proposal).$promise.then(
								...
								function(error) {
									Notification.error({
										...
										title:
											'<i class="fas fa-3x fa-exclamation-triangle"></i> Error - Assignment failed!'
									});
								}
							);
						}
			...
			function(
				...
				var _init = function() {
					...
					if (org)
						ppp.org.fullAddress =
							ppp.org.address +
							(ppp.org.address2 ? ', ' + ppp.org.address2 : '') +
							', ' +
							ppp.org.city +
							', ' +
							ppp.org.province +
					...
					ppp.validateInceptionAmount = function() {
						...
						if (
							ppp.proposal.phases.inception.cost < 0 ||
							ppp.proposal.phases.inception.cost > ppp.inceptionMax
						) {
						...
						if (
							ppp.proposal.phases.inception.cost +
								ppp.proposal.phases.proto.cost +
								ppp.proposal.phases.implementation.cost >
							ppp.opportunity.budget
						) {
					...
					ppp.validateImplementationAmount = function() {
						...
						if (
							ppp.proposal.phases.implementation.cost < 0 ||
							ppp.proposal.phases.implementation.cost > ppp.implMax
						) {
					...
					(ppp.tinymceOptions.toolbar =
						'undo redo | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent'),
					...
					ppp.inception.filteredItems =
						!ppp.inception.search || ppp.inception.search.length === 0
							...
							: ppp.members.inception.filter(function(member) {
									return (
										member.displayName.toUpperCase().includes(ppp.inception.search.toUpperCase()) ||
										ppp.inTeam[member.email] === true
									);
							  });
				};
					...
					ppp.poc.filteredItems =
						!ppp.poc.search || ppp.poc.search.length === 0
							...
							: ppp.members.proto.filter(function(member) {
									return (
										member.displayName.toUpperCase().includes(ppp.poc.search.toUpperCase()) ||
										ppp.prTeam[member.email] === true
									);
							  });
				};
						...
						!ppp.implementation.search || ppp.implementation.search.length === 0
							...
							: ppp.members.implementation.filter(function(member) {
									return (
										member.displayName
											.toUpperCase()
											.includes(ppp.implementation.search.toUpperCase()) ||
										ppp.imTeam[member.email] === true
									);
							  });
				};
				...
				var setSkills = function() {
					...
					if (ppp.oinp.isInception)
						ppp.inp.oppCapabilityCodes.forEach(function(c) {
							haveanyatall = true;
							var code = c;
					...
					if (ppp.oprp.isPrototype)
						ppp.prp.oppCapabilityCodes.forEach(function(c) {
							haveanyatall = true;
							var code = c;
					...
					if (ppp.oimp.isImplementation)
						ppp.imp.oppCapabilityCodes.forEach(function(c) {
							haveanyatall = true;
							var code = c;
				...
				var saveproposal = function(goodmessage, badmessage) {
					var validPriceAmounts =
						!ppp.exceededOpportunityAmount &&
						!ppp.proposal.phases.inception.invalidAmount &&
						!ppp.proposal.phases.proto.invalidAmount &&
						!ppp.proposal.phases.implementation.invalidAmount;
						...
						ppp.proposal.createOrUpdate().then(
							function(response) {
								Notification.success({
									message:
										goodmessage ||
										'<i class="fas fa-3x fa-check-circle"></i><br> <h4>Changes saved</h4>'
								});
								resolve();
							},
							function(error) {
				...
				ppp.delete = function() {
					performdelete(
						'Are you sure you want to delete your proposal? All your work will be lost. There is no undo for this!'
					);
				};
				// -------------------------------------------------------------------------
				//
				...
				ppp.submit = function() {
					...
					saveproposal('<h4>Your proposal has been submitted</h4>').then(function() {
						ppp.close();
					});
				};
				// -------------------------------------------------------------------------
						...
						Upload.upload({
							...
							function(response) {
								Notification.error({
									...
									title:
										'<i class="fas fa-3x fa-exclamation-triangle"></i> Error Uploading Attachment'
								});
							},
							function(evt) {
